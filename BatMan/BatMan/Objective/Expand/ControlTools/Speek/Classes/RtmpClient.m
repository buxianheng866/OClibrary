...
{
    int publishReconnectCount;
    int playReconnectCount;
}
@end
...
{
    //忽略SIGPIPE，否则如果此时socket管道破裂，则此信号可能会导致崩溃
    int set = 1;
    setsockopt(pRtmp->m_sb.sb_socket, SOL_SOCKET, SO_NOSIGPIPE, (void *)&set, sizeof(int));
...
-(id)initWithSampleRate:(int)sampleRate withEncoder:(int)audioEncoder
{
    ...
    if(self){
        ...
        if (!session)
        {
            printf("ERROR INITIALIZING AUDIO SESSION! \n");
        }
        else
        {
            NSError *nsError;
            ...
            if (nsError)
                printf("AudioSession setActive = YES failed");
        }
    }
    return self;
}
...
-(void)startPlayWithUrl:(NSString*)rtmpURL IsTeacher:(BOOL)isTeacher
{
    ...
    if (isTeacher) {
        ...
        // 学生 AVAudioSessionCategorySoloAmbient
...
-(void)resume{
    if (isStartPlay && mAudioPlayer && isPause) {
        [mAudioPlayer resume];
        isPause = NO;
    }
}
...
-(void)openPublishThread:(NSString*) rtmpUrl
{
    ...
    do {
        ...
        //添加
        if (!RTMP_Connect(pPubRtmp, NULL)) {
            NSLog(@"连接失败");
            publishReconnectCount ++;
            if(outDelegate)
            {
                [outDelegate EventCallback:1002];
            }
            ...
            publishReconnectCount = 0;
            if (!RTMP_ConnectStream(pPubRtmp, 0)) {
                NSLog(@"连接流失败");
                publishReconnectCount ++;
                if(outDelegate)
                {
...
//        }
    ...
    //
    if (publishReconnectCount > 0) {
        if (publishReconnectCount == 3) {
            publishReconnectCount = 0;
            // 提示发言失败
            [SVProgressHUD showImage:nil status:@"重连接失败"];
        }else{
            isStartPub = NO;
            ...
            // 销毁之前的
            [self openPublishThread:rtmpUrl];
            return;
        }
    }
...
-(void)openPlayThread:(NSString*) rtmpUrl
{
    ...
    do {
        ...
        //
        if (!RTMP_Connect(pPlayRtmp, NULL)) {
            NSLog(@"连接失败");
            playReconnectCount ++;
            if(outDelegate)
            {
            ...
            playReconnectCount = 0;
            if (!RTMP_ConnectStream(pPlayRtmp, 0)) {
                NSLog(@"连接流失败");
                playReconnectCount ++;
                if(outDelegate)
                {
...
//        }
        ...
        while (isStartPlay )
        {
            if(RTMP_ReadPacket(pPlayRtmp, &rtmp_pakt))
            {
...
//    [session setActive: NO error: nil];
    ...
    if (pPlayRtmp && RTMP_IsConnected(pPlayRtmp))
    {
    ...
    if (playReconnectCount > 0) {
        if (playReconnectCount == 3) {
            playReconnectCount = 0;
            // 提示发言失败
            [SVProgressHUD showImage:nil status:@"重连接失败"];
        }else{
            [self openPlayThread:rtmpUrl];
            return;
        }
    }
